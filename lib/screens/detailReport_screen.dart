...
class DetailReportScreen extends StatefulWidget {
  ...
  const DetailReportScreen({
    ...
    required this.data,
    required this.docId,
    this.isAdmin = false, // por defecto sin permisos
  });
...
class _DetailReportScreenState extends State<DetailReportScreen> {
  ...
  Future<void> _generatePdf() async {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          pw.Center(child: pw.Text("Reporte de Incendio", style: titleStyle)),
          ...
          buildSection("Estado del Reporte", [
            ...
            buildRow("Falsa alarma", data['falsa_alarma'] == true ? "Sí" : "No"),
          ]),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    final fecha =
        data['fecha_reporte']?.toString().split('T').first ?? 'Sin fecha';
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          buildInfoTile(Icons.local_activity, 'Tipo de servicio',
              data['tipo_servicio'] ?? ''),
          buildInfoTile(Icons.description, 'Descripción del servicio',
              data['descripcion_servicio'] ?? ''),
          buildInfoTile(
              Icons.phone, 'Tipo de llamada', data['tipo_llamada'] ?? ''),
          ...
          const Text('Ubicación',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(
              Icons.location_city, 'Ciudad', data['ciudad'] ?? ''),
          buildInfoTile(Icons.house, 'Colonia', data['colonia'] ?? ''),
          buildInfoTile(
              Icons.villa, 'Comunidad', data['comunidad'] ?? ''),
          buildInfoTile(Icons.aod, 'Carretera', data['carretera'] ?? ''),
          buildInfoTile(Icons.map, 'Dirección', data['direccion'] ?? ''),
          buildInfoTile(Icons.place, 'Descripción del lugar',
              data['descripcion_lugar'] ?? ''),
          buildInfoTile(Icons.navigation, 'Coordenadas N',
              data['coordenadas_n'] ?? ''),
          buildInfoTile(Icons.navigation, 'Coordenadas W',
              data['coordenadas_w'] ?? ''),
          ...
          const Text('Horarios',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(Icons.access_time, 'Hora del reporte',
              data['hr_reporte'] ?? ''),
          buildInfoTile(Icons.access_time, 'Hora salida base',
              data['hr_salida_base'] ?? ''),
          buildInfoTile(Icons.access_time, 'Hora llegada a escena',
              data['hr_arribo'] ?? ''),
          buildInfoTile(Icons.access_time, 'Hora salida de escena',
              data['hr_salida_escena'] ?? ''),
          buildInfoTile(Icons.access_time, 'Hora llegada a base',
              data['hr_llegada_base'] ?? ''),
          buildInfoTile(Icons.access_time, 'Hora unidad disponible',
              data['hr_unidad_disponible'] ?? ''),
          ...
          const Text('Unidad y personal',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(
              Icons.directions_car, 'Unidad', data['unidad'] ?? ''),
          buildInfoTile(Icons.directions_car, 'Operador',
              data['operador'] ?? ''),
          buildInfoTile(
              Icons.directions_car, 'Elabora', data['elabora'] ?? ''),
          buildInfoTile(Icons.directions_car, 'Jefe de servicio',
              data['jefe_servicio'] ?? ''),
          buildInfoTile(
              Icons.directions_car, 'Guardia', data['guardia'] ?? ''),
          buildInfoTile(
              Icons.directions_car, 'Función', data['funcion'] ?? ''),

          const SizedBox(height: 10),
          const Text('Reportante',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(Icons.account_circle_rounded, 'Reportante',
              data['reportante'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Teléfono',
              data['telefono'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Solicitado por',
              data['solicitado_por'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Nombre del afectado',
              data['nombre_afectado'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Destino',
              data['destino'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Razón social',
              data['razon_social'] ?? ''),
          ...
          const Text('Detalles adicionales',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(Icons.account_circle_rounded, 'Acciones',
              data['acciones'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Material',
              data['material'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Especificaciones',
              data['especifica'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Observaciones',
              data['observaciones'] ?? ''),
          buildInfoTile(
            Icons.account_circle_rounded,
            'Kilometraje',
            'Salida: ${data['km_salida'] ?? ''}, Llegada: ${data['km_llegada'] ?? ''}, Total: ${data['km'] ?? ''}',
          ),
          ...
          const Text('Datos administrativos',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(Icons.account_circle_rounded, 'Folio año',
              data['folio_ano'] ?? ''),
          buildInfoTile(Icons.account_circle_rounded, 'Folio C4',
              data['folio_c4'] ?? ''),
          buildInfoTile(Icons.local_activity, 'Tipo de servicio',
          ...
          const Text('Estado',
              style:
                  TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          buildInfoTile(Icons.cancel, 'Cancelado',
              data['cancelado'] == true ? 'Sí' : 'No'),
          buildInfoTile(Icons.warning_amber, 'Falsa alarma',
              data['falsa_alarma'] == true ? 'Sí' : 'No'),
